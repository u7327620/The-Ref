...
class GifView(discord.ui.View):
    ...
    async def approve_button_callback(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.client.get_channel(self.display_id).send(interaction.message.content)
...
class GifCog(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        if message.channel.id != self.submission_id:
            return
